...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    double rate_monthly = daily_rate(hourly_rate) * 22;
    return ceil(apply_discount(rate_monthly, discount));
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double rate_day = apply_discount(daily_rate(hourly_rate),discount);
    return int(budget/rate_day);
}
